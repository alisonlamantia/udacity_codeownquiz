...
#triggers which test text the game will use based on user input. It returns corresponding question and answers. If user doesn't select a level, the function is called again.
def levelchoice():
    ...
    if level == 'hard':
        return hardstring, answer
    else:
        print "please start over and pick level easy, medium or hard"
        levelchoice()

#runs the quiz by first calling levelchoice to determine the level the user will play
def quiz():
